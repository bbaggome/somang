...
DECLARE
  ...
  full_path TEXT;
BEGIN
  -- file_url에서 버킷 이름과 파일의 '전체 경로'를 추출합니다.
  SELECT
    regexp_matches[1],
    regexp_matches[2]
  INTO bucket, full_path
  FROM regexp_matches(OLD.file_url, '.*/object/public/([^/]+)/(.*)');

  -- 버킷과 경로가 정상적으로 추출된 경우에만 실행합니다.
  IF bucket IS NOT NULL AND full_path IS NOT NULL THEN
    -- `delete_prefix` 함수에 정확한 전체 경로를 전달하여 파일 하나만 삭제합니다.
    PERFORM storage.delete_prefix(bucket, full_path);
  END IF;
...
CREATE OR REPLACE FUNCTION "public"."generate_random_nickname"() RETURNS "text"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    adjectives text[] := ARRAY['멋진', '귀여운', '똑똑한', '재미있는', '활발한', '조용한', '친절한', '유쾌한', '신비한', '행복한', '용감한', '온화한', '밝은', '즐거운', '따뜻한'];
    nouns text[] := ARRAY['고양이', '강아지', '토끼', '판다', '코알라', '펭귄', '다람쥐', '햄스터', '여우', '늑대', '사자', '호랑이', '곰', '새', '물고기'];
...
ALTER FUNCTION "public"."generate_random_nickname"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."generate_unique_nickname"() RETURNS "text"
    LANGUAGE "plpgsql"
    AS $$
DECLARE
    adjectives text[] := ARRAY['멋진', '귀여운', '똑똑한', '재미있는', '활발한', '조용한', '친절한', '유쾌한', '신비한', '행복한', '용감한', '온화한', '밝은', '즐거운', '따뜻한', '상큼한', '깜찍한', '든든한', '포근한', '반짝이는'];
    nouns text[] := ARRAY['고양이', '강아지', '토끼', '판다', '코알라', '펭귄', '다람쥐', '햄스터', '여우', '늑대', '사자', '호랑이', '곰', '새', '물고기', '거북이', '돌고래', '나비', '꿀벌', '별'];
    random_adjective text;
    random_noun text;
    random_number text;
...
BEGIN
    LOOP
        ...
        -- 닉네임 조합
        generated_nickname := random_adjective || random_noun || random_number;
        
        -- 중복 확인
        SELECT EXISTS(
            SELECT 1 FROM public.profiles WHERE nick_name = generated_nickname
        ) INTO nickname_exists;
        
        -- 중복이 없으면 반환
        IF NOT nickname_exists THEN
        ...
        IF attempt_count >= max_attempts THEN
            -- 최대 시도 횟수 초과시 타임스탬프 추가
            RETURN generated_nickname || '_' || extract(epoch from now())::bigint;
        END IF;
    END LOOP;
END;
$$;
...
ALTER FUNCTION "public"."generate_unique_nickname"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_quote_request_details"("request_id_param" "uuid") RETURNS TABLE("quote_id" "uuid", "quote_created_at" timestamp with time zone, "quote_status" "text", "quote_details" "jsonb", "store_name" "text", "store_phone_number" "text", "store_base_address" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
...
BEGIN
  ...
  SELECT 
    ...
    s.base_address as store_base_address
  FROM quotes q
...
ALTER FUNCTION "public"."get_quote_request_details"("request_id_param" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."get_user_quotes"("user_id_param" "uuid") RETURNS TABLE("quote_id" "uuid", "quote_created_at" timestamp with time zone, "quote_status" "text", "quote_details" "jsonb", "request_id" "uuid", "request_details" "jsonb", "store_name" "text", "store_phone_number" "text", "store_base_address" "text")
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO 'public'
    AS $$
BEGIN
  ...
  WHERE qr.user_id = user_id_param
    AND qr.product_type = 'mobile_phone'
  ORDER BY q.created_at DESC;
END;
$$;


ALTER FUNCTION "public"."get_user_quotes"("user_id_param" "uuid") OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."handle_biz_signup"("p_user_id" "uuid", "p_name" "text", "p_biz_name" "text", "p_store_type" "text", "p_business_methods" "text"[], "p_phone_number" "text", "p_business_registration_number" "text", "p_pre_approval_number" "text", "p_zip_code" "text", "p_base_address" "text", "p_detail_address" "text", "p_document_file_url" "text", "p_seller_name" "text", "p_seller_phone_number" "text", "p_latitude" double precision, "p_longitude" double precision) RETURNS "uuid"
    LANGUAGE "plpgsql" SECURITY DEFINER
...
DECLARE
  new_store_id UUID;
BEGIN
  -- 1. profiles 테이블에 판매자 이름과 역할을 업데이트합니다.
  --    (판매자 개인 연락처는 보통 별도 컬럼에 저장하거나, 이 테이블에 저장할 경우 컬럼명을 확인해야 합니다.)
  UPDATE public.profiles
  SET
    name = p_seller_name, -- 혹은 full_name 등 컬럼명 확인
    role = 'owner'
  WHERE id = p_user_id;

  -- 2. 새로운 가게 정보를 'stores' 테이블에 추가합니다.
  ...
  VALUES (
    p_user_id,
    p_name,                           -- 누락되었던 p_name 인자를 사용
    p_biz_name,
    p_store_type,
    p_business_methods,
    p_phone_number,
    p_business_registration_number,
    p_pre_approval_number,
    p_zip_code,
    p_base_address,
    p_detail_address,
    'pending',                        -- 초기 상태는 '승인 대기'
    ST_SetSRID(ST_MakePoint(p_longitude, p_latitude), 4326)
  )
  RETURNING id INTO new_store_id;

  -- 3. 사업자등록증 파일 정보를 저장합니다.
  INSERT INTO public.store_verification_documents (store_id, document_type, file_url, status)
  VALUES (new_store_id, 'business_license', p_document_file_url, 'submitted');

  -- 4. 생성된 가게의 ID를 반환합니다.
  RETURN new_store_id;
EXCEPTION
  WHEN OTHERS THEN
    -- 오류 발생 시 명확한 에러 메시지를 반환합니다.
    RAISE EXCEPTION 'An error occurred in handle_biz_signup: %', SQLERRM;
END;
$$;


ALTER FUNCTION "public"."handle_biz_signup"("p_user_id" "uuid", "p_name" "text", "p_biz_name" "text", "p_store_type" "text", "p_business_methods" "text"[], "p_phone_number" "text", "p_business_registration_number" "text", "p_pre_approval_number" "text", "p_zip_code" "text", "p_base_address" "text", "p_detail_address" "text", "p_document_file_url" "text", "p_seller_name" "text", "p_seller_phone_number" "text", "p_latitude" double precision, "p_longitude" double precision) OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_new_user"() RETURNS "trigger"
    ...
    AS $$
BEGIN
  INSERT INTO public.profiles (id, name, email, avatar_url, nick_name)
  VALUES (
    NEW.id,
...
CREATE OR REPLACE FUNCTION "public"."send_quote_notification"() RETURNS "trigger"
    LANGUAGE "plpgsql"
    AS $$
  DECLARE
    user_subscriptions RECORD;
    req_details JSONB;  -- ✅ 변수명 변경 (request_details → req_details)
    device_info RECORD;
    notification_payload JSONB;
  BEGIN
    IF TG_OP = 'INSERT' THEN
      -- ✅ 테이블명 명시적 지정
      SELECT qr.request_details INTO req_details
      FROM quote_requests qr
      WHERE qr.id = NEW.request_id;

      -- 디바이스 정보 가져오기 (NULL 체크 추가)
      IF req_details->>'deviceId' IS NOT NULL THEN
        SELECT device_name, COALESCE(storage_options[1], 128) as storage
        INTO device_info
        FROM devices
        WHERE id = (req_details->>'deviceId')::UUID;
      END IF;

      -- 알림 페이로드 생성
      notification_payload := jsonb_build_object(
        'title', '새로운 견적이 도착했습니다! 📱',
        'body', COALESCE(device_info.device_name, '휴대폰') || ' ' ||
                COALESCE(device_info.storage::text, '128') || 'GB 견적을
  확인해보세요.',
        ...
        'data', jsonb_build_object(
          ...
          'url', '/quote/requests/' || NEW.request_id
        ),
        'requireInteraction', true
      );

      -- 간단한 로그만 남기고 실제 HTTP 요청은 제거 (일단 테스트용)
      RAISE NOTICE 'Quote notification triggered for request_id: %', NEW.request_id;       
...
ALTER FUNCTION "public"."send_quote_notification"() OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."devices" (
    ...
    "ram_gb" integer,
    "battery_mah" integer,
    "screen_size_inch" numeric(3,1),
    "display_spec" "text",
    "cpu_spec" "text",
    "camera_spec" "text",
    "external_memory_support" "text",
    "mobile_payment" "text",
    "water_dust_resistance" "text",
    "fingerprint_sensor" "text",
    "wireless_charging_supported" boolean,
    "factory_film_attached" boolean,
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
...
CREATE TABLE IF NOT EXISTS "public"."favorites" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "favorites_target_type_check" CHECK (("target_type" = 'STORE'::"text"))
);
...
CREATE TABLE IF NOT EXISTS "public"."internet_products" (
    "id" "uuid" DEFAULT "gen_random_uuid"() NOT NULL,
    ...
    "installation_fee_weekday" integer NOT NULL,
    "installation_fee_weekend" integer NOT NULL,
    "description" "text",
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "updated_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "deleted_at" timestamp with time zone,
    CONSTRAINT "internet_products_product_type_check" CHECK (("product_type" = ANY (ARRAY['internet_only'::"text", 'internet_tv'::"text"])))
);


ALTER TABLE "public"."internet_products" OWNER TO "postgres";
...
ALTER TABLE "public"."policy_consents" OWNER TO "postgres";
...
COMMENT ON COLUMN "public"."profiles"."nick_name" IS '사용자 닉네임 (랜덤 생성)';
...
CREATE TABLE IF NOT EXISTS "public"."quote_requests" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "deleted_at" timestamp with time zone,
    CONSTRAINT "quote_requests_product_type_check" CHECK (("product_type" = ANY (ARRAY['mobile_phone'::"text", 'internet'::"text"]))),
    CONSTRAINT "quote_requests_status_check" CHECK (("status" = ANY (ARRAY['open'::"text", 'closed'::"text", 'expired'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."quotes" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    "deleted_at" timestamp with time zone,
    CONSTRAINT "quotes_status_check" CHECK (("status" = ANY (ARRAY['sent'::"text", 'viewed'::"text", 'accepted'::"text", 'rejected'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."review_votes" (
    ...
    "created_at" timestamp with time zone DEFAULT "now"() NOT NULL,
    CONSTRAINT "review_votes_vote_type_check" CHECK (("vote_type" = ANY (ARRAY['like'::"text", 'dislike'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."store_verification_documents" (
    ...
    "reviewer_id" "uuid",
    "reject_reason" "text",
    CONSTRAINT "store_verification_documents_status_check" CHECK (("status" = ANY (ARRAY['submitted'::"text", 'approved'::"text", 'rejected'::"text"])))
);


ALTER TABLE "public"."store_verification_documents" OWNER TO "postgres";
...
CREATE TABLE IF NOT EXISTS "public"."stores" (
    ...
    "deleted_at" timestamp with time zone,
    "biz_name" "text",
    CONSTRAINT "stores_status_check" CHECK (("status" = ANY (ARRAY['pending'::"text", 'approved'::"text", 'rejected'::"text", 'suspended'::"text"])))
);
...
CREATE TABLE IF NOT EXISTS "public"."user_settings" (
    "user_id" "uuid" NOT NULL,
    "chat_notification_enabled" boolean DEFAULT true NOT NULL,
    "promotion_notification_enabled" boolean DEFAULT true NOT NULL,
...
ALTER TABLE ONLY "public"."internet_products"
    ADD CONSTRAINT "internet_products_pkey" PRIMARY KEY ("id");
...
ALTER TABLE ONLY "public"."policies"
    ADD CONSTRAINT "policies_policy_type_version_key" UNIQUE ("policy_type", "version");
...
ALTER TABLE ONLY "public"."policy_consents"
    ADD CONSTRAINT "policy_consents_user_id_policy_id_key" UNIQUE ("user_id", "policy_id");
...
ALTER TABLE ONLY "public"."reviews"
    ADD CONSTRAINT "reviews_quote_id_key" UNIQUE ("quote_id");



ALTER TABLE ONLY "public"."store_verification_documents"
    ADD CONSTRAINT "store_verification_documents_pkey" PRIMARY KEY ("id");



ALTER TABLE ONLY "public"."stores"
    ADD CONSTRAINT "stores_business_registration_number_key" UNIQUE ("business_registration_number");
...
ALTER TABLE ONLY "public"."user_push_subscriptions"
    ADD CONSTRAINT "uk_user_endpoint" UNIQUE ("user_id", "endpoint");



ALTER TABLE ONLY "public"."chat_rooms"
    ADD CONSTRAINT "unique_chat_per_context" UNIQUE ("user_id", "store_id", "quote_id");
...
ALTER TABLE ONLY "public"."user_push_subscriptions"
    ADD CONSTRAINT "user_push_subscriptions_user_id_unique" UNIQUE ("user_id");
...
CREATE INDEX "idx_user_push_subscriptions_active" ON "public"."user_push_subscriptions" USING "btree" ("is_active");
...
CREATE UNIQUE INDEX "idx_user_push_unique" ON "public"."user_push_subscriptions" USING "btree" ("user_id", "endpoint");
...
CREATE OR REPLACE TRIGGER "handle_devices_updated_at" BEFORE UPDATE ON "public"."devices" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();



CREATE OR REPLACE TRIGGER "handle_internet_products_updated_at" BEFORE UPDATE ON "public"."internet_products" FOR EACH ROW EXECUTE FUNCTION "public"."update_updated_at_column"();



CREATE OR REPLACE TRIGGER "quote_notification_trigger" AFTER INSERT ON "public"."quotes" FOR EACH ROW EXECUTE FUNCTION "public"."send_quote_notification"();
...
ALTER TABLE ONLY "public"."chat_rooms"
    ADD CONSTRAINT "chat_rooms_quote_id_fkey" FOREIGN KEY ("quote_id") REFERENCES "public"."quotes"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."chat_rooms"
    ADD CONSTRAINT "chat_rooms_store_id_fkey" FOREIGN KEY ("store_id") REFERENCES "public"."stores"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."chat_rooms"
    ADD CONSTRAINT "chat_rooms_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."notices"
    ADD CONSTRAINT "notices_admin_id_fkey" FOREIGN KEY ("admin_id") REFERENCES "public"."profiles"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."policy_consents"
    ADD CONSTRAINT "policy_consents_policy_id_fkey" FOREIGN KEY ("policy_id") REFERENCES "public"."policies"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."policy_consents"
    ADD CONSTRAINT "policy_consents_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."quote_requests"
    ADD CONSTRAINT "quote_requests_user_id_fkey" FOREIGN KEY ("user_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."quotes"
    ADD CONSTRAINT "quotes_request_id_fkey" FOREIGN KEY ("request_id") REFERENCES "public"."quote_requests"("id") ON DELETE CASCADE;
...
ALTER TABLE ONLY "public"."reviews"
    ADD CONSTRAINT "reviews_quote_id_fkey" FOREIGN KEY ("quote_id") REFERENCES "public"."quotes"("id") ON DELETE RESTRICT;



ALTER TABLE ONLY "public"."reviews"
    ADD CONSTRAINT "reviews_store_id_fkey" FOREIGN KEY ("store_id") REFERENCES "public"."stores"("id") ON DELETE SET NULL;
...
ALTER TABLE ONLY "public"."store_verification_documents"
    ADD CONSTRAINT "store_verification_documents_reviewer_id_fkey" FOREIGN KEY ("reviewer_id") REFERENCES "public"."profiles"("id") ON DELETE SET NULL;



ALTER TABLE ONLY "public"."store_verification_documents"
    ADD CONSTRAINT "store_verification_documents_store_id_fkey" FOREIGN KEY ("store_id") REFERENCES "public"."stores"("id") ON DELETE CASCADE;



ALTER TABLE ONLY "public"."stores"
    ADD CONSTRAINT "stores_owner_id_fkey" FOREIGN KEY ("owner_id") REFERENCES "public"."profiles"("id") ON DELETE RESTRICT;
...
ALTER TABLE ONLY "public"."ticket_replies"
    ADD CONSTRAINT "ticket_replies_replier_id_fkey" FOREIGN KEY ("replier_id") REFERENCES "public"."profiles"("id") ON DELETE CASCADE;
...
CREATE POLICY "Admins can do anything" ON "public"."stores" USING (( SELECT ("auth"."uid"() IN ( SELECT "profiles"."id"
           FROM "public"."profiles"
          WHERE ("profiles"."role" = 'admin'::"text")))));



CREATE POLICY "Owners can manage their own stores" ON "public"."stores" USING (("auth"."uid"() = "owner_id")) WITH CHECK (("auth"."uid"() = "owner_id"));



CREATE POLICY "Public can view approved stores" ON "public"."stores" FOR SELECT USING (("status" = 'approved'::"text"));
...
CREATE POLICY "Service role can access all subscriptions" ON "public"."user_push_subscriptions" USING ((("auth"."jwt"() ->> 'role'::"text") = 'service_role'::"text"));



CREATE POLICY "Store owners can create quotes" ON "public"."quotes" FOR INSERT WITH CHECK ((EXISTS ( SELECT 1
   FROM ("public"."stores"
     JOIN "public"."profiles" ON (("stores"."owner_id" = "profiles"."id")))
  WHERE (("stores"."id" = "quotes"."store_id") AND ("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'owner'::"text") AND ("stores"."deleted_at" IS NULL) AND ("profiles"."deleted_at" IS NULL)))));



CREATE POLICY "Store owners can update their quotes" ON "public"."quotes" FOR UPDATE USING ((EXISTS ( SELECT 1
   FROM ("public"."stores"
     JOIN "public"."profiles" ON (("stores"."owner_id" = "profiles"."id")))
  WHERE (("stores"."id" = "quotes"."store_id") AND ("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'owner'::"text") AND ("stores"."deleted_at" IS NULL) AND ("profiles"."deleted_at" IS NULL)))));
...
CREATE POLICY "Store owners can view push subscriptions for quotes" ON "public"."user_push_subscriptions" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM ((("public"."quote_requests" "qr"
     JOIN "public"."quotes" "q" ON (("q"."request_id" = "qr"."id")))
     JOIN "public"."stores" "s" ON (("s"."id" = "q"."store_id")))
     JOIN "public"."profiles" "p" ON (("p"."id" = "s"."owner_id")))
  WHERE (("qr"."user_id" = "user_push_subscriptions"."user_id") AND ("p"."id" = "auth"."uid"()) AND ("p"."role" = 'owner'::"text")))));



CREATE POLICY "Store owners can view their quotes" ON "public"."quotes" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM ("public"."stores"
     JOIN "public"."profiles" ON (("stores"."owner_id" = "profiles"."id")))
  WHERE (("stores"."id" = "quotes"."store_id") AND ("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'owner'::"text") AND ("stores"."deleted_at" IS NULL) AND ("profiles"."deleted_at" IS NULL)))));
...
CREATE POLICY "Users can delete own subscriptions" ON "public"."user_push_subscriptions" FOR DELETE USING (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can insert own quote requests" ON "public"."quote_requests" FOR INSERT WITH CHECK ((("auth"."uid"() = "user_id") AND (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'user'::"text"))))));



CREATE POLICY "Users can insert own subscriptions" ON "public"."user_push_subscriptions" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can insert their own quote requests" ON "public"."quote_requests" FOR INSERT WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can read own subscriptions" ON "public"."user_push_subscriptions" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can update own subscriptions" ON "public"."user_push_subscriptions" FOR UPDATE USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));



CREATE POLICY "Users can update their own quote requests" ON "public"."quote_requests" FOR UPDATE USING (("auth"."uid"() = "user_id")) WITH CHECK (("auth"."uid"() = "user_id"));
...
CREATE POLICY "Users can view own quote requests" ON "public"."quote_requests" FOR SELECT USING ((("auth"."uid"() = "user_id") OR (EXISTS ( SELECT 1
   FROM "public"."profiles"
  WHERE (("profiles"."id" = "auth"."uid"()) AND ("profiles"."role" = 'owner'::"text"))))));



CREATE POLICY "Users can view quotes for their requests" ON "public"."quotes" FOR SELECT USING ((EXISTS ( SELECT 1
   FROM "public"."quote_requests" "qr"
  WHERE (("qr"."id" = "quotes"."request_id") AND ("qr"."user_id" = "auth"."uid"())))));



CREATE POLICY "Users can view their own quote requests" ON "public"."quote_requests" FOR SELECT USING (("auth"."uid"() = "user_id"));
...
GRANT ALL ON FUNCTION "public"."generate_random_nickname"() TO "anon";
GRANT ALL ON FUNCTION "public"."generate_random_nickname"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."generate_random_nickname"() TO "service_role";



GRANT ALL ON FUNCTION "public"."generate_unique_nickname"() TO "anon";
GRANT ALL ON FUNCTION "public"."generate_unique_nickname"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."generate_unique_nickname"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."get_quote_request_details"("request_id_param" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_quote_request_details"("request_id_param" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_quote_request_details"("request_id_param" "uuid") TO "service_role";



GRANT ALL ON FUNCTION "public"."get_user_quotes"("user_id_param" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."get_user_quotes"("user_id_param" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."get_user_quotes"("user_id_param" "uuid") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."handle_biz_signup"("p_user_id" "uuid", "p_name" "text", "p_biz_name" "text", "p_store_type" "text", "p_business_methods" "text"[], "p_phone_number" "text", "p_business_registration_number" "text", "p_pre_approval_number" "text", "p_zip_code" "text", "p_base_address" "text", "p_detail_address" "text", "p_document_file_url" "text", "p_seller_name" "text", "p_seller_phone_number" "text", "p_latitude" double precision, "p_longitude" double precision) TO "anon";
GRANT ALL ON FUNCTION "public"."handle_biz_signup"("p_user_id" "uuid", "p_name" "text", "p_biz_name" "text", "p_store_type" "text", "p_business_methods" "text"[], "p_phone_number" "text", "p_business_registration_number" "text", "p_pre_approval_number" "text", "p_zip_code" "text", "p_base_address" "text", "p_detail_address" "text", "p_document_file_url" "text", "p_seller_name" "text", "p_seller_phone_number" "text", "p_latitude" double precision, "p_longitude" double precision) TO "authenticated";
GRANT ALL ON FUNCTION "public"."handle_biz_signup"("p_user_id" "uuid", "p_name" "text", "p_biz_name" "text", "p_store_type" "text", "p_business_methods" "text"[], "p_phone_number" "text", "p_business_registration_number" "text", "p_pre_approval_number" "text", "p_zip_code" "text", "p_base_address" "text", "p_detail_address" "text", "p_document_file_url" "text", "p_seller_name" "text", "p_seller_phone_number" "text", "p_latitude" double precision, "p_longitude" double precision) TO "service_role";
...
GRANT ALL ON FUNCTION "public"."send_quote_notification"() TO "anon";
GRANT ALL ON FUNCTION "public"."send_quote_notification"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."send_quote_notification"() TO "service_role";
...
GRANT ALL ON TABLE "public"."internet_products" TO "anon";
GRANT ALL ON TABLE "public"."internet_products" TO "authenticated";
GRANT ALL ON TABLE "public"."internet_products" TO "service_role";
...
GRANT ALL ON TABLE "public"."store_verification_documents" TO "anon";
GRANT ALL ON TABLE "public"."store_verification_documents" TO "authenticated";
GRANT ALL ON TABLE "public"."store_verification_documents" TO "service_role";
