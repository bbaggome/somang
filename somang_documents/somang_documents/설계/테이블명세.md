...
| `latitude`             | 위도        | `numeric(10,8)`|                                 | 주소의 위도 좌표. 지도 서비스에 사용됩니다.                                   |
| `longitude`            | 경도        | `numeric(11,8)`|                                 | 주소의 경도 좌표. 지도 서비스에 사용됩니다.                                   |
...
| `response_rate`        | 응답률       | `numeric(5,2)` | DEFAULT `0.00`                  | (캐시) 견적 응답률. 성능 최적화를 위해 주기적으로 계산하여 저장합니다.                     |
| `chat_room_count`      | 상담건수      | `integer`      | DEFAULT `0`                     | (캐시) 총 상담 건수.                                                 |
| `accepted_quote_count` | 개통건수      | `integer`      | DEFAULT `0`                     | (캐시) 총 개통(채택) 건수.                                             |
...
- 기타 설명: 하나의 거래(견적)에 대해 오직 하나의 리뷰만 작성 가능
...
- 기타 설명: 한 사용자는 하나의 리뷰에 대해 한 번만 투표할 수 있음
...
- 기타 설명: 동일한 사용자, 판매점, 견적 조합으로 중복된 채팅방이 생성되지 않음.
...
- 기타 설명: 한 사용자가 특정 버전의 약관에 대해 중복으로 동의하는 것을 방지.
