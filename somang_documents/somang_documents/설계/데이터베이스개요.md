...
18. **audit_logs**: 주요 시스템 활동에 대한 감사 로그 (신규 추가)
...
| `phone_number` | `text`                     | (신규) 판매자 연락처.                                            |
| `created_at` | `timestamp with time zone` | 프로필 생성 시각.                                              |
| `deleted_at` | `timestamp with time zone` | (신규) 프로필 소프트 삭제(soft delete) 시각. null이 아니면 삭제된 계정으로 간주. |
...
플랫폼에 입점한 통신 판매점(가맹점)의 상세 정보를 저장합니다. 캐시/비정규화된 컬럼은 DB 트리거를 통해 관리하는 것을 원칙으로 합니다.

| 컬럼 명                           | 데이터 타입                     | 설명                                                       |
| ------------------------------ | -------------------------- | -------------------------------------------------------- |
| **id**                         | `uuid` (PK)                | 판매점 고유 ID.                                               |
| `owner_id`                     | `uuid` (FK)                | 판매점주의 프로필 ID. `profiles.id`를 참조.                         |
| `name`                         | `text`                     | 판매점 상호명 (사업자상호명).                                        |
| `store_type`                   | `text`                     | (신규) 매장 구분.                                              |
| `business_method`              | `text`                     | (신규) 영업 방식.                                              |
| `phone_number`                 | `text`                     | (신규) 매장 연락처.                                             |
| `business_registration_number` | `text`                     | (신규) 사업자등록번호.                                            |
| `pre_approval_number`          | `text`                     | (신규) 사전승낙번호.                                             |
| `zip_code`                     | `text`                     | 우편번호.                                                    |
| `base_address`                 | `text`                     | 기본 주소.                                                   |
| `detail_address`               | `text`                     | 상세 주소.                                                   |
| `location`                     | `geography(Point, 4326)`   | (변경) 주소의 위도/경도 정보. PostGIS 확장을 통한 지리 공간 데이터.             |
...
| `response_rate`                | `numeric(5, 2)`            | (캐시) 견적 응답률. DB 트리거를 통해 주기적으로 계산하여 저장.                   |
| `chat_room_count`              | `integer`                  | (캐시) 총 상담 건수. DB 트리거로 관리.                                |
| `accepted_quote_count`         | `integer`                  | (캐시) 총 개통 건수. DB 트리거로 관리.                                |
| `created_at`                   | `timestamp with time zone` | 판매점 등록 시각.                                               |
| `deleted_at`                   | `timestamp with time zone` | (신규) 판매점 정보 소프트 삭제 시각.                                   |
...
최종 사용자가 원하는 통신 상품의 조건을 담아 제출하는 요청서 정보를 관리합니다. `request_details`의 JSON 스키마는 별도 문서로 정의하여 관리합니다.
...
| `deleted_at`      | `timestamp with time zone` | (신규) 요청 소프트 삭제 시각.                                        |

## 7. `quotes` - 표준 견적서

판매점이 사용자의 요청에 대해 제출하는 표준화된 견적서 정보를 저장합니다. `quote_details`의 JSON 스키마는 별도 문서로 정의하여 관리합니다.
...
| `deleted_at`    | `timestamp with time zone` | (신규) 견적 소프트 삭제 시각.                                            |
...
| `deleted_at` | `timestamp with time zone` | (신규) 리뷰 소프트 삭제 시각.                              |
...
## 18. `audit_logs` - 감사 로그 (신규 추가)

관리자 및 시스템의 주요 활동을 기록하여 추적성과 보안을 강화합니다.

| 컬럼 명       | 데이터 타입                  | 설명                                         |
| :------------ | :------------------------- | :------------------------------------------- |
| **id** | `uuid` (PK)                | 로그 레코드의 고유 ID.                           |
| `actor_id`    | `uuid` (FK)                | 작업을 수행한 주체(사용자/관리자)의 ID. `profiles.id`를 참조. |
| `action_type` | `text`                     | 수행된 작업의 종류 (예: 'STORE_APPROVED', 'USER_SUSPENDED'). |
| `target_id`   | `uuid`                     | 작업의 대상이 된 리소스의 ID (예: store_id, user_id).   |
| `old_value`   | `jsonb`                    | 변경 전 데이터 값 (필요시).                          |
| `new_value`   | `jsonb`                    | 변경 후 데이터 값 (필요시).                          |
| `created_at`  | `timestamp with time zone` | 작업이 수행된 시각.                              |